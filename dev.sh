...
GREEN='\033[00;32m'
YELLOW='\e[0;33m'
HOST_PROD=djawitter.example.com
...
workon djawitter  # Change this to your project's name
...
function dknpminstall {
    CD=$(pwd)
    cd $PROJ_BASE
    docker run -it --rm -v $(pwd):/app -w /app/frontend -e NODE_ENV=development djawitter npm install
    exitcode=$?
    cd $CD
    return $exitcode
}
...
function dkrun_prod {
    docker stop djawitter
    docker rm djawitter
    docker run --name djawitter -d --env-file /home/ubuntu/djawitter.env \
        -p 3000:3000 -p 8000:8000 \
        -v /home/ubuntu/dkdata/djawitter:/dkdata \
        djawitter start_web.sh
}
...
echo_green "Welcome to djawitter's dev env"
